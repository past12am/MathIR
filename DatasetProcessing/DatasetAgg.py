...
train_p = 0.8  # --> valid_p = 1 - train_p, no test set, because ARQMath provides test set
...
for q in tqdm(data):
    ...
    # Positive document: Concatenate multiple answers for the query
    positive_answers = random.sample(q['answers'], min(len(q['answers']), 3))  # Up to 3 answers
    positive_doc = " ".join([f"Answer {i+1}: {ans}" for i, ans in enumerate(positive_answers)])
    ...
    negative_answers = random.sample(d_b['answers'], min(len(d_b['answers']), 3))
    negative_doc = " ".join([f"Answer {i+1}: {ans}" for i, ans in enumerate(negative_answers)])
